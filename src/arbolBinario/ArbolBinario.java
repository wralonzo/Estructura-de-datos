...
public class ArbolBinario {
    private Nodo raiz;
    private int num_nodos;//numro de nodos
    private int alt;//altura
    ...
    public void insertar(int dato){
       ...
       Nodo nuevo = new Nodo(dato);//insstaciamos nodo
          if (raiz == null)
              raiz = nuevo;//si esta vacio crear uno nuevo
          else
          {//si existe el nodo raiz le agregamos los demas nodos
              Nodo anterior = null;
              Nodo tmp = raiz;
              while (tmp != null)
              {
                  anterior = tmp;
                  if (dato < tmp.getDato())//si es menor agregarlo a la izquierda
                      tmp = tmp.getIzq();
                  else
                      tmp = tmp.getDer();//si es mayor a la derecha
              }
    ...
    private void Max(Nodo aux, int max)
    {
         if (aux != null) {   
